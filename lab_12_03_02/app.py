...
def show_help():
    ...
    "при вводе больших значений возможны задержки выполнения").grid(row = len(text), column = 0, columnspan = 3, sticky = tk.W)
    tk.Label(new_window, text = "Разработчик : Дмитрий Варин ИУ7-36Б").grid(row = len(text) + 1, column = 0, sticky = tk.W)
...
def output(res):
    my_result.delete(0, my_result.size())
    for i in range(len(res)):
            my_result.insert(i, f"index = {i} elem = {str(res[i])}")
...
window = tk.Tk()
window.title("Работа с библиотекой импортированной из СИ")
window.resizable(width = False, height = False)

messagebox.showinfo("Предупреждение", "Перед началом работы соберите библиотеку\nВведите make lib из текущей директории")

tk.Label(text="Функция").grid(row=0, column=0, sticky=tk.W, pady=10, padx=10)
func_list = ttk.Combobox(values=["Массив простых","Фильтрация массива_1","Фильтрация массива_2",],postcommand=change_func)
...
my_result = tk.Listbox(width = 60, height = 4)
my_result.grid(row = 4, column = 1, columnspan = 3) 
